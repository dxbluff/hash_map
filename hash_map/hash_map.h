#pragma once
#include <list>
#define SIZE 100 //default massiv's size 

template<class keytype, class valtype>
class hash_map {
	std::list<std::pair<keytype, valtype> > *table; //massiv of elements;
	int size = SIZE; //size of table
	int count=0;  //count of elements
public:
	hash_map();
	hash_map(int _s); // _s probably quantities of elements
	void insert(keytype key, valtype val); //adding new item with index=hash of key
	void show(keytype key); //for test------------
	void clear(); //clean hash_map
	void erase(keytype k); //delete element
	int get_size(); //return's count of elements
	bool empty(); //return's "1" if container is empty
